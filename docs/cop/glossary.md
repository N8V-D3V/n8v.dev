# COP Glossary

Version: 0.3.0

---

## Contract

A human-readable document that defines system behavior, inputs, outputs, success, failure, and constraints.

---

## Protocol

An interface that represents a capability required to fulfill a contract.

---

## Module

An implementation that fulfills one or more protocols.

---

## Orchestrator

A component that coordinates modules to execute system behavior.

---

## Agent

An AI or human role responsible for performing a specific function within the COP system.

---

## Artifact

A structured output produced during the development process, such as a contract, protocol specification, or validation report.

---

## Source Of Truth

The authoritative definition of system behavior.

In COP, this is always the contract.

---

## Validation

The process of ensuring implementation matches the contract.

---

## Constraint

A rule that must always be followed.

---

## Edge Case

A non-standard or uncommon scenario that must still be handled correctly.

---

## Failure Mode

A defined way the system can fail and how it must respond.

---

## Stub

A temporary implementation that simulates contract-defined behavior without performing the final real logic or external integration.
